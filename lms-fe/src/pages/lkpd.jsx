      ...
      <main className="grow flex items-center justify-center p-4 md:p-8 relative z-10"> {/* flex-grow diubah menjadi grow */}
        <div className="w-full max-w-4xl bg-white p-6 md:p-10 lg:p-12 rounded-4xl shadow-2xl transition duration-500 hover:shadow-3xl transform hover:scale-[1.005]"> {/* rounded-[2rem] diubah menjadi rounded-4xl */}
          ...
          <div className="mb-8 pt-4">
            <h2 className="text-3xl md:text-4xl font-extrabold text-[#1F6294] mb-2">LKPD</h2>
            <div className="w-full h-px bg-gray-200"></div>
          </div>
          
          {/* Langkah Pengerjaan LKPD */}
          <div className="mb-10 p-5 rounded-xl bg-green-50/70 border border-green-200">
            <h3 className="text-xl md:text-2xl font-bold text-[#1F6294] mb-4">Langkah Pengerjaan LKPD</h3>
            
            <ol className="list-decimal list-inside space-y-3 pl-2">
              {langkahPengerjaan.map((langkah, index) => (
                <li key={index} className="text-sm md:text-base text-gray-700 leading-relaxed font-medium">
                  {/* Menghapus bullet default dan menggunakan angka dari list */}
                  <span className="font-normal">{langkah}</span>
                </li>
              ))}
            </ol>
          </div>
